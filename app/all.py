...
def add():
    for i in range(random.randint(101,999)):
        carid = "101" + str(i)
    ...
    if request.method == "POST":
        ...
        cur.execute("INSERT INTO car(carmodel,fueltype,transmission,cost,carpic,id) VALUES(%s,%s,%s,%s,%s,%s)",(carmodel,fueltype,transmission,cost,carpic,carid))
...
def modify(id):
    cur = mysql.connection.cursor()
    cur.execute("select * from car where id = '"+str(id)+"'")
    car = cur.fetchone()
    mysql.connection.commit()
    cur.close()
    print(car)
    return render_template("base/display.html")
